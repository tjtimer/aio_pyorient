...
from aio_pyorient.constants import (FIELD_BOOLEAN, FIELD_BYTE, FIELD_BYTES, FIELD_INT, FIELD_LONG, FIELD_SHORT,
                                    FIELD_STRING, RECORD_CREATE_OP, RECORD_DELETE_OP, RECORD_LOAD_OP, RECORD_TYPES,
                                    RECORD_TYPE_DOCUMENT, RECORD_UPDATE_OP)
from aio_pyorient.exceptions import (
...
class RecordCreateMessage(BaseMessage):
    ...
    async def fetch_response(self):
        ...
        if self.protocol > 21:
            try:
                chng = await self._decode_field(FIELD_INT)
                """ count-of-collection-changes """
            except ( PyOrientConnectionException, TypeError ):
                pass

            try:
                if chng > 0 and self.protocol > 23:

                    for x in range( 0, chng ):
                        change = [
                            await self._decode_field(FIELD_LONG),  # (uuid-most-sig-bits:long)
                            await self._decode_field(FIELD_LONG),  # (uuid-least-sig-bits:long)
                            await self._decode_field(FIELD_LONG),  # (updated-file-id:long)
                            await self._decode_field(FIELD_LONG),  # (updated-page-index:long)
                            await self._decode_field(FIELD_INT)  # (updated-page-offset:int)
                        ]
...
class RecordLoadMessage(BaseMessage):
    ...
    async def fetch_response(self):
        ...
        _status = list(await super().fetch_response())[0]
        ...
        if _status != 0:
            ...
            __record = await super().fetch_response(True)
            ...
            await self._read_async_records()  # get cache
