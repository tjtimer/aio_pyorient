...
from aio_pyorient.utils import is_debug_active
...
class BaseMessage(object):
    ...
    def __init__(self, sock=OrientSocket, *,
        ...
        """
        assert sock.connected, """OrientSocket instance must be connected."""
        self._loop = loop
    ...
    def _set_connected(self, *args):
        print(f"BaseMessage _set_connected args: {args}")
        self._protocol = self._connection.protocol
        self._session_id = self._connection.session_id

        # handles token auth
        self._auth_token = self._connection.auth_token

        self._db_opened = self._connection.db_opened
