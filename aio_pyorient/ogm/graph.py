...
from .commands import CreateEdgeCommand, VertexCommand
...
class Graph(object):
    ...
    def __init__(self, config, user=None, cred=None, strict=False, *,
        ...
        connection_task.add_done_callback(self._is_connected.set)
        self._tasks["connect"] = connection_task

        self.config = config

        if config.initial_drop:
            self._last_db = self._last_user = self._last_cred = None
            self._loop.create_task(self.drop())
        ...
        self.scripts = config.scripts or aio_pyorient.Scripts()
    ...
    async def open(self, db_name, storage, user=None, cred=None):
        ...
        if not self.client.db_exists(db_name, storage):
            await self.client.db_create(
                db_name, aio_pyorient.DB_TYPE_GRAPH, storage
            )
    ...
    async def drop(self, db_name=None, storage=None):
        """Drop entire database."""
        config = self.config
        await self.client.connect(config.user, config.cred)
        try:
            dropped_db = db_name or config.db_name
            await self.client.db_drop(
                db_name or config.db_name, storage or config.storage
            )
        except:
            return False
        finally:
    ...
    def build_mapping(self, vertex, edge, auto_plural=False):
        ...
        schema = self.client.command(
            ...
            WHERE name NOT IN [
              \'ORole\', \'ORestricted\', \'OTriggered\', \'ORIDs\',
              \'OUser\', \'OIdentity\', \'OSchedule\', \'OFunction\'
              ]"""
        )
    ...
    def create_class(self, cls):
        ...
        except aio_pyorient.PyOrientSchemaException:
        ...
        for prop_name, prop_value in props:
                ...
                self.client.command(
                    'CREATE PROPERTY {0} {1} {2}'
                        .format(class_prop
                                ...
                                , linked_to or ''))
            except aio_pyorient.PyOrientCommandException:
            ...
            if prop_value.indexed:
                ...
                except aio_pyorient.PyOrientIndexException:
    ...
    def edge_from_record(self, record, edge_cls = None):
        ...
        if type(in_prop) is aio_pyorient.OrientRecordLink:
            in_hash = in_prop.get_hash()

        out_hash = None
        out_prop = props['out']
        if type(out_prop) is aio_pyorient.OrientRecordLink:
    ...
    def element_from_record(self, record):
        if not isinstance(record, aio_pyorient.OrientRecord):
            return record

        record_data = record.oRecordData
        try:
            if isinstance(record_data['in'], aio_pyorient.OrientRecordLink) and \
                    isinstance(record_data['out'],
                               aio_pyorient.OrientRecordLink):
    ...
    def coerce_class_names(classes):
        ...
        return getattr(classes, 'registry_name', classes)
