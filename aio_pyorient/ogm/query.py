...
from .operators import (ArithmeticOperation, IdentityOperand, LogicalConnective, Operator)
from .property import Property, PropertyEncoder
from .query_utils import ArgConverter
from .what import ChainableWhat, FunctionWhat, QV, What
...
class Query(object):
    ...
    def parse_record_prop(self, prop):
        if isinstance(prop, list):
            g = self._graph
            return g.elements_from_links(prop) if len(prop) > 0 and isinstance(prop[0],
                                                                               aio_pyorient.OrientRecordLink) else prop
        elif isinstance(prop, aio_pyorient.OrientRecordLink):
